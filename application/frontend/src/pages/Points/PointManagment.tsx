...
export default function PointManagment() {
  ...
  useEffect(() => {
    if (userClaims.role != 0) {
      ...
      fetch(`http://http://ec2-54-221-146-123.compute-1.amazonaws.com:3333/drivers/u:${userClaims.id}`)
        .then((res) => res.json())
        .then((data) => {
          setPointsRatio({ PointsRatio: data.Organization.PointsRatio })
        })
    ...
    if (userClaims.role != 0) {
      setTimeout(() => {
        if (window.confirm('Do you want to accept this change?')) {
          ...
          fetch('http://http://ec2-54-221-146-123.compute-1.amazonaws.com:3333/points/category', requestOptions)
